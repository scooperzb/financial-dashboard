...
BLOCK_TRADES_FILE = Path(__file__).parent / "block_trades.json"
...
def render_block_trade_metrics(trades_df: pd.DataFrame,
    ...
    if not trades_df.empty:
        ...
        if ps:
            try:
                ...
                start = dt.strptime(ps, "%Y-%m-%d")
                period_label = f" — {start.strftime('%B %Y')}"
            except ValueError:
                pass

        st.markdown(
            f'<div class="section-header">Block Trades{period_label}</div>',
            unsafe_allow_html=True,
        )
        st.caption(f"Last updated: {bt_meta.get('last_updated', '—')}")

        # Summary metrics
        st.markdown(
            render_block_trade_metrics(trades_df, switches_df, fx_rate),
            unsafe_allow_html=True,
        )
        ...
        for _, trade in displayable.iterrows():
            ...
            html = render_trade_card(trade.to_dict(), sw_label, current_price)
